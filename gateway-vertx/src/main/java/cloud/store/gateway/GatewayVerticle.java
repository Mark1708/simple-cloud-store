...
public class GatewayVerticle extends AbstractVerticle {
    ...
    public static void main(String[] args) {
        VertxApplication.main(new String[] {"run", GatewayVerticle.class.getName()});
    }
    ...
    public void start() {
        ...
        if (logFactory == null) {
            System.setProperty(
                    "org.vertx.logger-delegate-factory-class-name", "io.vertx.core.logging.SLF4JLogDelegateFactory");
        }
    ...
    private Router getRouter() {
        ...
        router.route().handler(CorsHandler.create().addOrigin("*").allowedMethod(HttpMethod.GET));
    ...
    private WebClient getClient(String host, Integer port, String serviceName) {
        WebClientOptions inventoryOptions =
    ...
    private Single<JsonObject> getAvailabilityFromInventory(JsonObject product) {
        return inventory
                ...
                .map(resp -> {
                    if (resp.statusCode() != 200) {
                        log.warn(
                                "Inventory error for {}: status code {}",
                                product.getString("itemId"),
                                resp.statusCode());
                        return product.copy();
                    }
                    return product.copy()
